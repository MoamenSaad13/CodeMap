...
/**
 * @description Check if all lessons in a stage are completed by a user.
 * @param {String} userId - The ID of the user.
 * @param {String} stageId - The ID of the stage to check.
 * @returns {Promise<Object>} - Returns an object with the result of the completion check.
 */
const checkStageCompletion = async (userId, stageId) => {
  try {
    const stage = await Stage.findById(stageId).populate("lesson roadmap");
    if (!stage) {
    ...
    const totalLessons = stage.lesson.length;
    const completedLessons = stage.lesson.filter(lesson => 
      user.completedlesson.includes(lesson._id)
    ).length;
    ...
    return {
      success: false,
      message: "Error checking stage completion",
      error: error.message
    };
  }
};
...
/**
 * @description Check if all lessons and tasks in a roadmap are completed by a user.
 * @param {String} userId - The ID of the user.
 * @param {String} roadmapId - The ID of the roadmap to check.
 * @returns {Promise<Object>} - Returns an object with the result of the completion check.
 */
const checkRoadmapCompletion = async (userId, roadmapId) => {
  try {
    const roadmap = await Roadmap.findById(roadmapId).populate("lesson stage");
    if (!roadmap) {
    ...
    // Check if all lessons in the roadmap are completed
    const totalLessons = roadmap.lesson.length;
    const completedLessons = roadmap.lesson.filter(lesson => 
      user.completedlesson.includes(lesson._id)
    ).length;

    // Also check if all tasks in the roadmap are completed
    const allTasks = await Task.find({ 
      category: { $in: roadmap.category }
    });
    
    const Submission = require("../models/Submission");
    const completedTasks = await Submission.find({
      user: userId,
      task: { $in: allTasks.map(task => task._id) },
    ...
    console.error("Error in checkRoadmapCompletion:", error);
    return {
      success: false,
      message: "Error checking roadmap completion",
      error: error.message
    };
  }
};
 ...
 */
const generateTaskFromQuestionPool = async (userId, categoryId) => { // Removed session parameter as it's now outside main transaction
  try {
    console.log("Generating task for user:", userId, "category:", categoryId);
...
const newTaskData = {
 ...
 questions: selectedQuestions.map(q => ({
  ...
  requireAllLessonsCompleted: true,
    ...
    // 10) Push the Task reference onto User, Category, and QuestionPool
...
const markLessonAsCompleted = async (req, res) => {
  ...
  let session = null; // Initialize session outside try-catch for broader scope
...
if (!user.roadmap.includes(currentLesson.roadmap.toString())) {
  ...
  return res.status(403).json({
    ...
    // --- Mark Lesson as Completed (Primary Transaction) ---
    ...
    // Commit the transaction for lesson completion here
    await session.commitTransaction();
    console.log("Transaction committed successfully (lesson marked completed).");
    session.endSession(); // End the session for the primary transaction
    session = null; // Clear session to prevent accidental reuse

    // --- Respond to client immediately after lesson is marked completed ---
    res.status(200).json({
      message: "Lesson marked as completed successfully.",
      taskGenerated: false, // Default to false, will be updated if task is generated
    });
    responseSent = true; // Mark response as sent

    // --- Asynchronous Operations (outside the main transaction) ---
    // These operations will not affect the lesson completion status if they fail
    try {
      // Check Category Completion and Generate Task if Needed
      const checkResult = await checkCategoryCompletion(userId, currentLesson.category);
      console.log("Check result for category completion (after commit):", checkResult);

      if (checkResult.success && checkResult.isCompleted) {
        console.log("Category completed, attempting to generate task...");
        const genResult = await generateTaskFromQuestionPool(userId, currentLesson.category);

        if (genResult.success) {
          console.log("Task generation successful.");
          // If response was already sent, we can't update status code, but can log/notify
          // For this scenario, we'll just log success and not try to modify the response
        } else {
          console.error("Task generation failed (async):");
          console.error("Error:", genResult.message);
          // Send a notification to admin or log this failure for review
        }
      }
    } catch (taskGenError) {
      console.error("Error during asynchronous task generation:", taskGenError);
    }

    // Achievement Notifications: Check Stage Completion
    try {
      const stageCheckResult = await checkStageCompletion(userId, currentLesson.stage);
      if (stageCheckResult.success && stageCheckResult.isCompleted) {
        await NotificationService.createNotification({
          type: "achievement",
          title: "🎉 Stage Completed!",
          message: `Congratulations ${user.first_name}! You have successfully completed the "${stageCheckResult.stage.title}" stage in "${stageCheckResult.stage.roadmap.title}". Keep up the excellent work!`,
          assignedTo: userId,
          relatedRoadmap: stageCheckResult.stage.roadmap._id,
          actions: [
            {
              label: "Continue to Next Stage",
              action: "view",
              url: `/roadmaps/${stageCheckResult.stage.roadmap._id}`,
              style: "success",
            },
            {
              label: "View Progress",
          ...
          metadata: {
            achievementType: "stage_completion",
            stageTitle: stageCheckResult.stage.title,
            roadmapTitle: stageCheckResult.stage.roadmap.title,
            completionDate: new Date(),
            lessonsCompleted: stageCheckResult.totalCount
          }
        });
        console.log(`Stage completion achievement sent to user: ${user.email} for stage: ${stageCheckResult.stage.title}`);
      }
    } catch (notificationError) {
      console.error("Error sending stage completion notification:", notificationError);
    }

    // Achievement Notifications: Check Roadmap Completion
    try {
      const roadmapCheckResult = await checkRoadmapCompletion(userId, currentLesson.roadmap);
      if (roadmapCheckResult.success && roadmapCheckResult.isCompleted) {
        await NotificationService.createNotification({
          type: "achievement",
          title: "🏆 Roadmap Mastered!",
          message: `Outstanding achievement ${user.first_name}! You have successfully completed the entire "${roadmapCheckResult.roadmap.title}" roadmap. You've mastered ${roadmapCheckResult.totalLessons} lessons and ${roadmapCheckResult.totalTasks} tasks. This is a significant milestone in your learning journey!`,
          assignedTo: userId,
          relatedRoadmap: currentLesson.roadmap,
          actions: [
            {
              label: "Explore New Roadmaps",
              action: "view",
              url: "/roadmaps",
              style: "success",
            },
            {
              ...
              action: "view",
              url: `/certificates/${currentLesson.roadmap}`,
              style: "primary",
            },
          ],
          metadata: {
            achievementType: "roadmap_completion",
            roadmapTitle: roadmapCheckResult.roadmap.title,
            completionDate: new Date(),
            lessonsCompleted: roadmapCheckResult.totalLessons,
            tasksCompleted: roadmapCheckResult.totalTasks,
            stagesCompleted: roadmapCheckResult.roadmap.stage.length
          }
        });
        console.log(`Roadmap completion achievement sent to user: ${user.email} for roadmap: ${roadmapCheckResult.roadmap.title}`);
      }
    } catch (notificationError) {
      console.error("Error sending roadmap completion notification:", notificationError);
    }
    ...
    if (session && session.inTransaction() && !responseSent) {
      try {
        await session.abortTransaction();
    ...
    if (!responseSent) {
        if (session) session.endSession(); // End session if it was started and not ended
...
const deleteLesson = async (req, res) => {
  ...
  try {
    ...
    if (!deletedLesson) {
      return res.status(404).json({ message: "Lesson not inot found." });
    }
